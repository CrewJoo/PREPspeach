...
const analysisSchema = z.object({
    ...
    overallFeedback: z.string().describe("지원자에 대한 냉정하고 객관적인 종합 평가 (500자 내외). 면접관의 시선에서 합격 가능성이나 치명적인 약점 등을 직설적으로 언급할 것."),
});
...
export async function POST(req: Request) {
    ...
    const context = `
    [Interview Mode]: ${mode === 'SCHOOL' ? 'University Admission (Student)' : 'Job Interview (Candidate)'}
    ...
    const systemPrompt = `
    # Role
    You are an expert Interview Coach (HR Specialist or Admissions Officer).
    ...
    **Extremely Cold, Objective, and Professional.**
    ...
    4. [Overall Feedback] Provide a comprehensive, cold, and objective review of the candidate. Mention specific strengths but focus on critical weaknesses or areas that need significant improvement to pass the interview.
