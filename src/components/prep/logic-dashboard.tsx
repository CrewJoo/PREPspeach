...
const TierIcons: Record<Tier, React.ReactNode> = {
    Seed: <Leaf className="w-8 h-8 text-green-300" />,
    Sprout: <Sprout className="w-8 h-8 text-green-500" />,
    Branch: <TreeDeciduous className="w-8 h-8 text-green-600" />,
    Tree: <TreePine className="w-8 h-8 text-green-700" />,
    Forest: <Mountain className="w-8 h-8 text-green-800" />,
};
...
export function LogicDashboard() {
    ...
    // If next is null (Forest), progress is 100
    // Otherwise, current tier threshold is previous milestone.
    // Let's simplify: progress = (currentScore / nextTierTotal) * 100 ? No that's not range based.
    // Actually simplicity: Just show score. Or if we want range:
    // We need 'prevThreshold'. 
    // Seed: 0~100. Progress = score.
    // Sprout: 100~300. Progress = (score-100)/(300-100)*100.
    ...
    if (!nextInfo.next) {
        ...
        // Wait, simpler:
        // currentScore. 
        // nextInfo.total is the Target Score.
        // We assume Tier ranges are hardcoded.
        let base = 0;
        let target = 100;
    ...
    return (
        <motion.div
            ...
            className="w-full bg-white rounded-2xl shadow-lg border border-slate-100 p-6 sm:p-8 mb-10 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden"
        >
            ...
            <div className="flex flex-col items-center gap-2">
                ...
                <div className="bg-slate-800 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
            ...
            <div className="flex-1 w-full text-center md:text-left space-y-4">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-sm font-bold text-slate-400">
                        <span>현재 XP: <span className="text-slate-700">{totalScore}</span></span>
                        {nextInfo.next && <span>다음 티어까지 {nextInfo.remaining} XP</span>}
                        {!nextInfo.next && <span>최고 레벨 도달!</span>}
                    </div>
            ...
            <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
                <div className="bg-slate-50 p-4 rounded-xl text-center border border-slate-100">
                    <div className="text-2xl font-black text-slate-700">{totalScore}</div>
                    <div className="text-xs text-slate-400 font-bold">TOTAL SCORE</div>
                </div>
                {/* Could add total solved bunches count if available in store easy access */}
            </div>
        </motion.div>
    );
}
